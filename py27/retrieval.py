...
parser.add_argument(
    "--f", type=str, default="frame/weather/train/", help="retrieval directory"
)
...
def demo(img_name, index, im, bb_array, gt, best_index, output_dir):
    ...
    for single_bb in xrange(top):
        if single_bb == 0:
            ...
            ax.text(
                bb_array[single_bb][0] + 6,
                bb_array[single_bb][1] - 14,
        ...
        for iteration in xrange(iteration_max):
            for img_name in os.listdir(frame_dir):
                        ...
                        for layer in layer_list:
                            for visual_feature in visual_feature_list:
                                pkl_name = os.path.join(
                                    visual_dir, visual_feature, bb_dir, layer, file_name
                                )
                                try:
                                    pkl = pickle.load(open(pkl_name, "rb"))
                                except:
                                    IPython.embed()
                                bb_proposal_pkl[visual_feature + "_" + layer].append(
                                    pkl.reshape(np.product(pkl.shape))
                                )
                        proposal_num += 1
                ...
                with open(poi_dir + img_name + ".txt", "r") as f:
                    center_pos = [0, 0]
                    [center_pos[0], center_pos[1], yaw, frame_height] = frame_info[
                        img_name
                    ]
                    easy_num = 0
                    ...
                    for line in f:
                        ...
                        map_vector = math.degrees(
                            similarity.angle(
                                [0, 1],
                                similarity.gpsvecotr(
                                    Point(gps_pos[1], gps_pos[0]),
                        ...
                        for query in query_list:
                            for layer in layer_list:
                                for visual_feature in visual_feature_list:
                                    ...
                                    visual_dis_list.append(
                                        scipy.spatial.distance.cdist(
                                            target_pkl, bb_proposal_array
                                        )
                                    )
                        ...
                        for bb_index in xrange(len(bb_proposal)):
                            ...
                            drone_angle = math.degrees(
                                similarity.angle(drone_middle, drone_vector)
                            )
                            ...
                            if iou >= iou_threshold:
                                distance_x_dis = similarity.likelydistance(
                                    gps_pos,
                                    center_pos,
                                    bb,
                                    height,
                                    width,
                                    similarity.disofpoint2line(
                                        p1, origin_p, similarity.yawtovector(yaw)
                                    ),
                                    True,
                                    True,
                                )
                                distance_y_dis = similarity.likelydistance(
                                    gps_pos,
                                    center_pos,
                                    bb,
                                    height,
                        ...
                        for single_bb in xrange(len(bb_array)):
                            matching_pair.append(
                                [
                                    ans_index,
                                    single_bb,
                                    bb_array[single_bb][0:4],
                                    bb_array[single_bb][4],
                                ]
                            )
